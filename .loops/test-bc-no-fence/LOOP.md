# Loop: test-bc-no-fence

## Pattern
Builder/Checker Loop
写代码和查代码拆成两个Agent，编排器循环调度，查到全绿为止。三文件模式：builder.md + checker.md + loop编排器
...
| 阶段 | 能力 | 状态 |
|------|------|------|
| L1 只报告 | builder只读分析，checker只报告（不修改） |  |
| L2 辅助修复 | builder写代码，checker跑检查，循环到ALL GREEN或停止条件触发 | ✓ 当前 |
| L3 无人值守 | 无人值守循环+自动提PR（需denylist和严格停止规则） |  |
