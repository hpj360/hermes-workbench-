# Loop: test-precheck-needs-human-fallback

## Pattern
Knowledge Hygiene
定期清理知识库：过期文档、重复skill、intent debt检测、偿还三笔债
...
| 阶段 | 能力 | 状态 |
|------|------|------|
| L1 只报告 | 只报告：过期文档、重复skill、缺失的项目约定 | ✓ 当前 |
| L2 辅助修复 | 更新时间戳、标记重复、整理索引 |  |
| L3 无人值守 | （不建议自动删除）提示用户确认后清理 |  |
